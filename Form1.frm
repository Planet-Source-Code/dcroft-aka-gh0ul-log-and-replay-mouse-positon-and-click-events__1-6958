...
Private Sub Timer1_Timer()
    ...
    ' button down
    If DontCount < 50 Then Exit Sub
    ' check to see if a click event was sent
    If GetAsyncKeyState(VK_LBUTTON) Then
        ' Left Mouse button down
        Label5 = "Left mouse button down"
        cp(idx).click_x = pt.x
        cp(idx).click_y = pt.y
        cp(idx).Button = BT_LEFT
        cp(idx).dblClicked = False
    ElseIf GetAsyncKeyState(VK_RBUTTON) Then
        ' Right mouse button down
        Label5 = "Right mouse button down"
        cp(idx).click_x = pt.x
        cp(idx).click_y = pt.y
        cp(idx).Button = BT_RIGHT
        cp(idx).dblClicked = False
    End If
...
Private Sub Timer2_Timer()
    ...
    ' met stop and re-show the cursor
    ...
    If cp(idx).click_x <> 0 And cp(idx).click_y <> 0 Then
       ...
       If cp(idx).Button = BT_LEFT Then
          ...
          Label5 = "Left button clicked (" & cp(idx).click_x & "," & cp(idx).click_y & ")"
       ElseIf cp(idx).Button = BT_RIGHT Then
          ...
          Label5 = "Right button  clicked (" & cp(idx).click_x & "," & cp(idx).click_y & ")"
       End If
...
Private Sub DOAbout()
   ...
   msg = vbCrLf
   msg = msg & "Cursor Logger v.1.25" & vbCrLf
   msg = msg & "-----------------------------------" & vbCrLf
   msg = msg & vbCrLf
   ...
   msg = msg & "Improvements:" & vbCrLf
   msg = msg & "----------------------------------" & vbCrLf
   msg = msg & vbCrLf
   msg = msg & "1.) New GUI" & vbCrLf
   msg = msg & "2.) Variable sized array usage for smaller log files "
   msg = msg & "and extended usage time." & vbCrLf
   msg = msg & "3.) Ability to replay older files." & vbCrLf
   msg = msg & "4.) Ability to name saved files." & vbCrLf & vbCrLf
   
   msg = msg & "Future Improvemens:" & vbCrLf
   msg = msg & "----------------------------------" & vbCrLf
   msg = msg & vbCrLf
   msg = msg & "Catching the Double Click event." & vbCrLf
   msg = msg & vbCrLf
